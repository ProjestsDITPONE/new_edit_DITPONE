...
const ScrollableTabBar = createReactClass({
  ...
  renderTab(name, page, isTabActive, onPressHandler, onLayoutHandler) {
    ...
    const textColor = isTabActive ? '#2d6dc4' : '#2d6dc4';
    const fontWeight = '';
    ...
    return (
      ...
      //   colors={isTabActive ? [ '#5dbde6','#1d61bd'] : ['#FFFFFF', '#FFFFFF']}
      //    style={{ borderWidth: Platform.OS === 'android' ? 1:0,
      //   borderColor: Platform.OS === 'android' && "#2d6dc4"}}
      // >
      <Button
        style={[styles.bttStyle,{backgroundColor: isTabActive?'#F3FBFF':'#FFF'}]}
        key={`${name}_${page}`}
        accessible={true}
        ...
        {/* <LinearGradient
          ...
          <View
            style={([styles.tab, this.props.tabStyle], {marginHorizontal: 15,})}>
            <Text
              style={[
                {
                  color: textColor,
                  fontWeight,
                  top: Platform.OS === 'android' && 6,
                },
                textStyle,
              ]}>
              {name} 
