...
import Style from '../../page/MarketContry/Style';
...
export default class SearchableDropDown extends Component {
  ...
  searchedItems = searchedText => {
    ...
    if (!setSort && typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        return (
          item.name_en.toLowerCase().indexOf(searchedText.toLowerCase()) > -1
        );
      };
    }
      ...
      return (
        <TouchableOpacity
          ...
          this.props.selectedItems.find(x => x.id === item.id) ? (
            <Text style={{...this.props.itemTextStyle}}>{item.name_en}</Text>
          ) : (
            <View>
              <Text style={{...this.props.itemTextStyle}}>{item.name_en}</Text>
              <View style={{alignItems: 'center'}}>
                <Image
                  style={{width: 245, height: 2}}
                  source={require('../../image/lineSeach10.png')}
                />
              </View>
            </View>
          )}
  ...
  renderTextInput = () => {
    ...
    return (
      <View style={Style.ViewSubSearch8}>
        <View style={Style.ViewSubSearch3}>
          <Image
            style={{width: 15, height: 15}}
            source={require('../../image/unsearch.png')}
          />
        </View>
  ...
  //               key={index}
  //               style={{
  //                 width: item.name_en.length * 8 + 60,
  //                 justifyContent: 'center',
  //                 flex: 0,
  //                 backgroundColor: '#eee',
  //                 flexDirection: 'row',
  ...
  //                 borderRadius: 15,
  //               }}>
  //               <Text style={{color: '##20416e'}}>{item.name_en}</Text>
  //               <TouchableOpacity
  //                 onPress={() =>
  //                   setTimeout(() => {
  //                     this.props.onRemoveItem(item, index);
