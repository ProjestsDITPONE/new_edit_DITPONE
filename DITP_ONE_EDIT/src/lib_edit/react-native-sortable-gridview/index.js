...
export default class SortableGridview extends Component {
  ...
  _caluIndex = (x, y) => {
    ...
    let columNum = Math.floor(
      centerX / (this.state.perWidth + this.state.gapWidth),
    );
    let rowNum = Math.floor(
      centerY / (this.state.perHeight + this.state.gapWidth),
    );
    ...
    index =
      index > this.state.data.length - 1 ? this.state.data.length - 1 : index;
  ...
  _setContentStyle = width => {
    ...
    const perWidth =
      (width - (numPerRow - 1) * gapWidth - (paddingLeft + paddingRight)) /
      numPerRow;
    ...
    if (this.positions.length !== allData.length) {
      ...
      allData.map((item, index) => {
        ...
        this.positions[index == 2 ? 5 : index == 5 ? 2 : index] = {
    ...
    return (
      <ScrollView
        ...
        <View
          ...
          {this.positions &&
            this.positions.length === allData.length &&
            this.state.data.map((item, index) => {
              const selectStyle =
                this.state.selectIndex === index ? this.state.selectStyle : {};
              const content = this.props.renderItem(item, index);
              const customTap = content.props.onTap
                ? content.props.onTap
                : () => {};
              const key = content.props.uniqueKey || index;

              if (!this[`moveAnimate${key}`] || this.moveAnimate) {
                this[`moveAnimate${key}`] = new Animated.ValueXY(
              ...
              return [
                <Animated.View
                  ...
                  style={[
                    styles.absolute,
                    {
                      elevation: this.state.selectIndex === index ? 10 : 1,
                      height: this.state.perHeight,
                      width: this.state.perWidth,
                      zIndex: this.state.selectIndex === index ? 10 : 1,
                      ...selectStyle,
                      ...this._getAnimation(key),
                    },
