...
class AnalyzeAi extends React.Component {
  constructor(props) {
    const getDate = new Date();
    super(props);
    this.state = {
      ...
      searchAttributeproduct: 'name',
      searchByTitle: false,
      ignoreCase: true,
      searchByTitle1: false,
      ignoreCase1: true,
  ...
  _getSeachCountry = async value => {
    try {
      ...
      if (response.res_code === '00') {
        // alert('DDD'+JSON.stringify(response.res_results));
        this.setState({Continent: response.res_results});
      }
    } catch (error) {}
  };

  _getProduct = async value => {
    try {
      var tokenActivity = '';
      ...
      const response = await this.props.dispatch(getDataProductnews(payload));
      // alert('DDD'+JSON.stringify(response.res_result));
      if (response.res_code === '00') {
        this.setState({Productnews: response.res_result});
      }
      this.arrayholder4 = this.state.Productnews;
    } catch (error) {}
  };
  ...
  SelecitemCT = ({item, index}) => {
    // alert(JSON.stringify(index))
    // alert(  this.state.itemCT[item.id] )

    let {CheckBoxCT, itemCT, checkBoxCT} = this.state;
    CheckBoxCT[index] = !CheckBoxCT[index];
    this.setState({CheckBoxCT: CheckBoxCT});
    if (CheckBoxCT[index] === true) {
      // alert(CheckBoxCT[index])
      itemCT.push({
        '': item.name.toString(),
      });
    } else {
      // alert(CheckBoxCT[index])
      // this.setState({CheckBoxCT: false});
      itemCT.pop(item.id);
    }
    console.log('itemMaketitemCT', this.state.itemCT);
  };

  //   this.setState({searchcontry: index});
  // };
  SelecitemProduct = ({item, index}) => {
    //  alert('ประเทศ'+item+index);

    this.setState({searchProducts: item, ckindexs: index});
  };
  ...
  selecitem5 = ({item, index}) => {
    let {checkBox, itemnameproducts, DataType, CheckBoxAll5} = this.state;

    // console.log()
    // this.state.itemnameproducts[index]?.name+"===="+item.name
    ...
    if (checkBox[index] === true) {
      ...
      itemnameproducts.push({
        name: item.name.toString(),
      });
    } else {
      this.setState({CheckBoxAll5: false});
      itemnameproducts.pop(item.name);
    }

    console.log(this.state.itemnameproducts);
  };

  yearCount() {
    var start_year = new Date().getFullYear();
    // alert(start_year);
    for (var yearDef = start_year; yearDef >= start_year - 1; yearDef--) {
      if (I18n.locale === 'th') {
  ...
  _getDataNewsAi = async values => {
    //  consoloe.log(this.state.textproductS)

    if (!this.state.fetching_from_server && !this.state.isListEnd) {
      this.setState({fetching_from_server: true}, async () => {
        const num = parseInt(this.state.valueSelectYYYY);
        //  alert(num - 543);
        const YYs = new Date().getFullYear();
        const MMs = new Date().getMonth();
        const DDs = new Date().getDate();
        //  alert(DDs);
        try {
          // console.log('values.countryvalues.countryvaluesNews' + this.state.Searchcountry);
            ...
            getDataNewsAi({
              result: {
                ...
                month:
                  ...
                  this.state.valueSelectMouthindex === 0
                    ? ''
                    : this.state.valueSelectMouthindex,
                day:
                  this.state.valueSelectDDDD === undefined ||
                  this.state.valueSelectDDDD === 'วัน'
                    ? ''
                    : this.state.valueSelectDDDD,
                sentiment:
                  this.state.sortnews !== null
                    ? this.state.sortnews === 1
                      ? [
                          'Slightly Negative',
                          'Moderately Negative',
                          'Highly Negative',
                    ...
                    : [],
                // country: this.state.searchcontry,
                country: this.state.itemCT,
                regions:
                  this.state.Searchcountry === undefined
                    ? null
                    : this.state.Searchcountry,
                // regions:this.state.Searchcountry === undefined ? null : this.state.Searchcountry ,
                products: this.state.itemnameproducts,
                text_search: this.state.textproductS,
              },
              token: tokenActivity,
            }),
          );
          if (this.response.res_text === 'success !!') {
            // console.log('showDATA', this.response.result);
            console.log('UCUCUCUCUCUCU' + this.response.result.length);
            if (this.response.result.length > 0) {
              // alert('true')
              this.offset = this.offset + 1;
              // console.log('State', this.state.DatanewsAi);
              this.setState({
                DatanewsAi: [...this.state.DatanewsAi, ...this.response.result],
                fetching_from_server: false,
                CheckRecom: [...this.state.DatanewsAi, ...this.response.result],
              });
              this.state.DatanewsAi.map(data => {
                this.state.Selec[data.new_ID] = data.status_basket;
              });

              this.arrayholder3 = this.state.DatanewsAi;
            } else {
  ...
  selecitem = ({index, item}) => {
    ...
    Selec[item.new_ID] = !Selec[item.new_ID];
    this.setState({Selec: Selec});
    if (Selec[item.new_ID] === true) {
      return this._SendBasket({code: item.new_ID});
    } else {
      return this._DeleteBasket({code: item.new_ID});
    }
  };

  selecitem2 = ({index, item}) => {
    let {Selec2, DataTopPick} = this.state;
    Selec2[item.new_ID] = !Selec2[item.new_ID];
    this.setState({Selec2: Selec2});
    if (Selec2[item.new_ID] === true) {
      return this._SendBasket({code: item.new_ID});
    } else {
      return this._DeleteBasket({code: item.new_ID});
    }
  ...
  SearchSubmit = e => {
    ...
    this.setState({DatanewsAi: newData});
  };

  SearchSubmitproduct = e => {
    const newData = this.arrayholder4.filter(item => {
      const itemData = `${item.name.toUpperCase()}${item.name.toUpperCase()}`;
      const textData = e.toUpperCase();
      return itemData.indexOf(textData) > -1;
    });

    this.setState({Productnews: newData});
  };
              ...
              <View style={{flexDirection: 'row'}}>
                ...
                <View
                  ...
                  <CheckBox
                    ...
                    checked={this.state.Selec[item.new_ID]}
                    onPress={() => {
                  ...
                  />
                    ...
                    checked={this.state.Selec2[item.new_ID]}
                    onPress={() => {
          ...
          subtitle={
            <View>
              ...
              <View
                ...
                <View style={[Styles.ViewSub10, {marginHorizontal: 2}]}>
                ...
                <View style={[Styles.ViewSub10, {marginHorizontal: 2}]}>
                  <Text
                    style={{color: '#6f819a', fontSize: 14, letterSpacing: 1}}>
  ...
  renderFooter() {
    return (
      <View
        style={[
          Styles.footer,
          {marginTop: this.state.DatanewsAi.length == 0 ? height * 0.1 : 0},
        ]}>
        {this.state.fetching_from_server === true ? (
          ...
          <View>
            {this.state.DatanewsAi.length == 0 ? (
  ...
  render() {
    const {
      ...
      searchByTitle,
      searchTermProduct1,
      searchAttributeproduct,
      ignoreCase1,
      searchByTitle1,
    } = this.state;
    // console.log('this.state.countDay');
    // console.log(this.state.Productnews);

    return (
      <View style={{flex: 1, backgroundColor: '#FFFFFF', zIndex: -1}}>
        <Headers
          badgeNumber="2"
        ...
        {/* <ScrollView style={{flex: 1, zIndex: -1}}> */}
          ...
          <Headerstage nameTab={I18n.t('transalte_analyze')} />
          ...
          <View style={{flex: 1}}>
            ...
            <FlatList
              ...
              data={this.state.DatanewsAi}
              
               onEndReached={() =>  this._getDataNewsAi()} //เป็นเพราะว่าผมใช้ตัวนี้ครับ
              ...
              ListHeaderComponent={
                ...
                <View style={styles.container}>
                 <Svg
                   ...
                   <G
                     onPressIn={() => {
                       // alert(this.state.selectSearchcountry);
                       // if(selected === "Africa" && )
                       // onSelectContinent('Africa');
                       this.state.selectSearchcountry === null
                         ? this.setState({selectSearchcountry: 'Africa'})
                         : this.setState({selectSearchcountry: ''});
     
                       this.state.selected !== 'Africa'
                         ? this.setState({selected: 'Africa'})
                         : this.setState({selected: ''});
     
                       this.setState({Searchcountry: 'Africa'});
                       ...
                       this.setState(
                         ...
                         function() {
                           this.offset = 0;
                           this._getDataNewsAi({country: 'South America'});
                         },
                       );
     
                       // this.setState({DatanewsAi: [], closePopover: false});
                       // this._getDataNewsAi({country: 'Africa'});
                     }}>
                     <Path
                       fill={
                         this.state.selected === 'Africa' ? '#54b2a9' : '#83d0c9'
                       }
                       id="path4307"
                     ...
                     <View style={{marginTop: height * 0.11 }}>
                       <View style={{marginTop: Platform.OS === 'android' ? 25 : null}}>
                       <Text style={{color: '#568ae0', textAlign: 'center', fontFamily: 'Mitr-Regular', fontWeight: "bold"}}>
                        {I18n.locale === 'th' ? 'อเมริกา' : 'Africa'}
                       </Text>
                       </View>
                     </View>
                   </G>
                   ...
                   <G
                     onPressIn={() => {
                       ...
                       this.state.selected !== 'South America'
                         ? this.setState({selected: 'South America'})
                         : this.setState({selected: '', country: ''});
     
                       this.setState({Searchcountry: 'South America'});
                       ...
                       // this.setState({DatanewsAi: [], closePopover: false});
                       // this._getDataNewsAi({country: 'Australia'});
                     }}>
                     <View
                       style={{
                         marginTop:
                           Platform.OS === 'ios' ? height * 0.15 : height * null,
                       }}>
                       <Text style={{color: '#568ae0', marginLeft: 65 ,fontFamily: 'Mitr-Regular', fontWeight: "bold"}}>
                        {I18n.locale === 'th' ? 'อเมริกาใต้' : 'South America'}
                       </Text>
                     </View>
                     <Path
                       fill={
                   ...
                   <G
                     onPressIn={() => {
                       ...
                       this.state.selected !== 'Europe'
                         ? this.setState({selected: 'Europe'})
                         : this.setState({selected: ''});
     
                       // this.setState({DatanewsAi: [], closePopover: false});
                       // this._getDataNewsAi({country: 'Europe'});
                       this.setState({Searchcountry: 'Europe'});
                       this.setState(
                         {
                           // ...this.state,
                           itemCT:[],
                     ...
                     <View
                       style={{
                         marginTop:
                           Platform.OS === 'ios' ? height * 0.04 : height * -0.14,
                       }}>
                       <Text style={{color: '#568ae0', textAlign: 'center',fontFamily: 'Mitr-Regular', fontWeight: "bold"}}>
                         {I18n.locale === 'th' ? 'ยูโรป' : 'Europe'}
                       </Text>
                     </View>
                     <Path
                       fill={
                         this.state.selected === 'Europe' ? '#54b2a9' : '#83d0c9'
                       }
                       id="path5896"
                   ...
                   <G
                     onPressIn={() => {
                       // onSelectContinent('Asia');
                       // selected !== 'Asia' ? setSelected('Asia') : setSelected('')
     
                       this.state.selected !== 'Asia'
                         ? this.setState({selected: 'Asia'})
                         : this.setState({selected: ''});
     
                       this.setState({Searchcountry: 'Asia'});
                       ...
                       this.setState(
                         ...
                         function() {
                           this.offset = 0;
                           this._getDataNewsAi({country: 'Asia'});//call API
                         },
                       );
     
                       // this.setState({DatanewsAi: [], closePopover: false});
                       // this._getDataNewsAi({country: 'Asia'});
                     }}>
                     <View
                       style={{
                         marginTop:
                           Platform.OS === 'ios' ? height * 0.06 : height * -0.12,
                       }}>
                       <Text
                         style={{
                           color: '#568ae0',
                         ...
                         {I18n.locale === 'th' ? 'เอเชีย' : 'Asia'}
                       </Text>
                     </View>
     
                     <Path
                       fill={this.state.selected === 'Asia' ? '#54b2a9' : '#83d0c9'}
                       id="path5928"
                   ...
                   <G
                     onPressIn={() => {
                       // onSelectContinent('North America');
     
                       this.state.selected !== 'North America'
                         ? this.setState({selected: 'North America'})
                         : this.setState({selected: ''});
     
                       this.setState({Searchcountry: 'North America'});
     
                       // this.setState({DatanewsAi: [], closePopover: false});
                       // this._getDataNewsAi({country: 'North America'});
                       this.setState(
                         {
                           // ...this.state,
                           itemCT:[],
                     ...
                     <View
                       ...
                       <Text style={{color: '#568ae0', marginLeft: 30,fontFamily: 'Mitr-Regular', fontWeight: "bold"}}>
                         {I18n.locale === 'th' ? 'อเมริกาเหนือ' : 'North America'}
                       </Text>
                     </View>
                     <Path
                       fill={
                   ...
                   <G
                     onPressIn={() => {
                       // onSelectContinent('Australia');
     
                       this.state.selected !== 'Australia'
                         ? this.setState({selected: 'Australia'})
                         : this.setState({selected: ''});
     
                       this.setState({Searchcountry: 'Oceania'});
                       ...
                       this.setState(
                         ...
                         function() {
                           this.offset = 0;
                           this._getDataNewsAi({country: 'Oceania'});
                         },
                       );
                     }}>
                     <View
                       style={{
                         marginTop:
                           Platform.OS === 'ios' ? height * 0.17 : height * 0.01,
                       }}>
                       <Text
                         style={{
                           color: '#568ae0',
                         ...
                         {I18n.locale === 'th' ? 'ออสเตเลีย' : 'Australia'}
                       </Text>
                     </View>
                     <Path
                       fill={
                         this.state.selected === 'Australia' ? '#54b2a9' : '#83d0c9'
                       }
                       id="path5922"
               ...
               <View
                 ...
                 <View
                   ...
                   <Text>{I18n.t('translate_negative_news')}</Text>
                 </View>
                 <View
                   style={{
                     borderWidth: 1,
                 ...
                 <View
                   style={{fontSize: 12, color: '#4b4b4b', marginHorizontal: 5}}>
                   <Text>{I18n.t('translate_positive_news')}</Text>
                 </View>
               </View>
                   ...
                   <View style={[Styles.ViewSub2]}>
                     ...
                     <TextInput
                       ...
                       placeholderTextColor={
                         this.state.selected === '' ? '#dadada' : '#568ae0'
                       }
                       style={[
                       ...
                       value={this.state.textproductS}
                       placeholder={
                         this.state.selected === ''
                           ? I18n.t('translate_Seach')
                           : this.state.selected
                       }
                     />
                   </View>
                     ...
                     this.setState(
                       {
                        valueSelectMMMM:'เดือน',
                         valueSelectYYYY: undefined,
   
                         valueSelectMouthindex: 0,
                         selected: '',
                         valueSelectDDDD: 'วัน',
                         sortnews: null,
                         // country: this.state.searchcontry,
         ...
         sortnewstext:null,
               ...
               <View
                     ...
                     <TouchableOpacity
                       ...
                       <View style={Styles.view1}>
                         <View style={Styles.view2}>
                           <Text style={Styles.viewtext3}>{I18n.t('transalte_tradeActivities_search_time')}</Text>
                         </View>
                   ...
                   <View
                     ...
                     <View
                       ...
                       <View
                         ...
                         <RNPickerSelect
                           ...
                           onDonePress={(value, index) => {
                             //uuuuuuuuuu
                             // alert(this.state.valueSelectYYYY);
                             this.setState({
                               valueSelectYYYY:
                                 this.state.valueSelectYYYY === undefined
                                   ? this.state.setdefulttear
                           ...
                           <View style={Styles.inview1}>
                             <View style={Styles.inview2}>
                               <Text style={Styles.inviewtext3}>
                                 {this.state.valueSelectYYYY === undefined
                                   ? '   ' + I18n.locale === 'th' ? 'ปี' : 'Year'
                                   : '   ' + this.state.valueSelectYYYY}
                               </Text>
                             </View>
                             <View style={Styles.inviewicon4}>
                               <Icon
                                 name="chevron-down"
                                 size={20}
                                 color="#2d6dc4"
                       ...
                       <View
                         ...
                         <RNPickerSelect
                           ...
                           onValueChange={(value, index) =>
                             this.setState({
                               valueSelectMMMM: value,
                               valueSelectMouthindex: index,
                             })
                           }
                           onDonePress={() => {
                           ...
                           <View style={Styles.inview1}>
                             <View style={Styles.inview2}>
                               <Text style={Styles.inviewtext3}>
                                 {this.state.valueSelectMouthindex === undefined
                                   ? '   ' + I18n.locale === 'th' ? 'เดือน' : 'Month'
                                   : '   ' + this.state.valueSelectMMMM}
                               </Text>
                             </View>
                             <View style={[Styles.inviewicon4, {flex: 0.25}]}>
                               <Icon
                                 name="chevron-down"
                                 size={20}
                                 color="#2d6dc4"
                       ...
                       <View
                         ...
                         <RNPickerSelect
                           ...
                           placeholder={{
                             label: I18n.locale === 'th' ? 'วัน' : 'Choose Day',
                             value: I18n.locale === 'th' ? 'วัน' : 'Choose Day',
                           }}
                           value={
                             this.state.valueSelectDDDD === undefined
                     ...
                     <TouchableOpacity
                       onPress={() => {
                         //dddddddddddd
                         // alert();
                         // alert( this.state.valueSelectYYYY +"\nMMM"+this.state.valueSelectMouthindex +"\n"+this.state.valueSelectDDDD)
                         this.setState(
                           {
                             ...this.state,
                             closePopover: false,
                 ...
                 <TouchableOpacity
                   onPress={() => {
                     this.setState({ChooseProducts: true});
                   }}
                   style={{
                     flex: 0.5,
                     marginHorizontal: 10,
                 ...
                 <TouchableOpacity
                   onPress={() => {
                     this.setState({selectcontry: true});
                   }}
                   style={{
                     flex: 0.5,
                     marginHorizontal: 10,
             ...
             <View
                 ...
                 <DropDownPicker
                   ...
                   style={[Styles.StyeSelec]}
                   searchableStyle={Styles.backgroundColor}
                   selectedLabelStyle={Styles.fontSelec}
                   placeholderStyle={Styles.fontSelecP}
                   arrowStyle={[Styles.ArrowStyle]}
                   activeItemStyle={{justifyContent: 'center'}}
                   labelStyle={Styles.fontSelec}
                   placeholder={I18n.t('translate_SORTBY')}
                   defaultValue={this.setState((this.state.Sort = null))}
                   items={[
                     {label: I18n.locale === 'th' ? 'เชิงลบ' : 'Negative', value: 1},
                     // {label: I18n.t('translate_Oldest_Date'), value: 2},
                     {label: I18n.locale === 'th' ? 'เชิงบวก' : 'Positive', value: 2},
             ...
             onScrollToTop={(info)=>console.log(info)}
                  ...
                  {item.status && this.state.togleUser[item.id_main] && (
                    <View
                      ...
                      <CheckBox
                        ...
                        checked={this.state.CheckBoxCT[item.id]}
                        onPress={() => {
                          this.SelecitemCT({index: item.id, item: item});
        ...
        {this.state.ChooseProducts == true && (
          <Overlay
            backdropStyle={{backgroundColor: '#2d6dc480'}}
            overlayStyle={{
              // borderWidth: 1,
            ...
            isVisible={this.state.ChooseProducts}
            onBackdropPress={() => this.setState({ChooseProducts: false})}>
            <View
              style={{
                flexDirection: 'row',
                borderWidth: 1,
