...
class ChatScreen extends Component {
  ...
  GetHistory = async value => {
    try {
     const oldtime = this.props.route.params.datatime
      var today = new Date();
      var dd = String(today.getDate()).padStart(2, '0');
      var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
      var yyyy = today.getFullYear();

      var dayStart = oldtime + 'T00:00:00.000Z';
      var dayend = oldtime + 'T23:59:59.999Z';

      // alert(dayend)

      const uid = this.props.getUser.userDetails.res_result.naturalId;
      const payload = {
        accessToken: 'H7eTTrQSdVUJLAqzpMoxsrw4YAV24ZEm',
        limit: 100,
        page: 0,
        start: dayStart,
        end: dayend,
      ...
      console.log(JSON.stringify(response.res_result));
      this.setState({
    ...
    try {
      ...
      if (response.res_code == '00') {
        ...
        if (response1.activateBot === true) {
          ...
          this.socket = SocketIOClient('wss://prod-mojito-gateway.socialenable.co', {
            path: '/chat-socket/socket.io',
            query: {
              token: response1.res_result,
            },
